...
- [Python package polars-ds](https://github.com/abstractqqq/polars_ds_extension) - A modern take on data science and traditional tabular machine learning with Polars. It provides Python modules that helps with EDA tasks for Polars dataframes, machine learning pipelines that are native to Polars (cleaner and faster than Sklearn). In addition, it contains a plugin that provides many common numerical, statistical functions, least squares, KNN queries, and many other ML metrics as expressions executable inside a Polars dataframe.[@abstractqqq](https://github.com/abstractqqq).
